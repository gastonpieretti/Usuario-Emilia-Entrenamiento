/**
 * RoutineBlueprintEngine
 * Defines weekly structure before selecting exercises.
 * Aligned with motor_entrenamiento.json v1.0
 */

export interface BlueprintDay {
    dia: number;
    enfoque: string[];
    patrones: string[];
    tipo_letra: string; // A, B, C, D
}

export interface BlueprintOutput {
    blueprint: BlueprintDay[];
}

const PLANTILLAS_DEFINICION: Record<string, { enfoque: string[], patrones: string[] }> = {
    "A": {
        "enfoque": ["piernas", "core"],
        "patrones": ["sentadilla", "bisagra", "core"]
    },
    "B": {
        "enfoque": ["empuje", "traccion"],
        "patrones": ["empuje", "traccion"]
    },
    "C": {
        "enfoque": ["full_body"],
        "patrones": ["sentadilla", "empuje", "traccion"]
    },
    "D": {
        "enfoque": ["metabolico"],
        "patrones": ["circuito"]
    }
};

export const generateWeeklyBlueprint = (daysPerWeek: number, goal: string, level: string): BlueprintOutput => {
    let structure: string[] = [];

    if (daysPerWeek === 3) {
        structure = ["A", "B", "A"];
    } else if (daysPerWeek === 4) {
        structure = ["A", "B", "A", "C"];
    } else if (daysPerWeek >= 5) {
        structure = ["A", "B", "C", "A", "D"];
    }

    const blueprint: BlueprintDay[] = structure.map((tipo, index) => {
        const def = PLANTILLAS_DEFINICION[tipo];
        return {
            dia: index + 1,
            tipo_letra: tipo,
            enfoque: def.enfoque,
            patrones: def.patrones
        };
    });

    return { blueprint };
};
